...
//方法主要逻辑是遍历节点的发送消息集合 vSendMsg，
//然后调用 send 方法发送每一个消息，并针对发送正确与否进行处理；同时从发送消息集合 vSendMsg 对应的消息。
            ...
            if (recvSet || errorSet)
            {
                ...
                if (nBytes > 0)
                {
                    ...
                    if (!pnode->ReceiveMsgBytes(pchBuf, nBytes, notify))
                        pnode->CloseSocketDisconnect();
                    RecordBytesRecv(nBytes);//    nTotalBytesRecv += bytes;
                    if (notify) {
                        size_t nSizeAdded = 0;
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Tsc " + FormatFullVersion();
...
{
 	LogPrint(BCLog::NET,"ThreadMessageHandler()\n");
    while (!flagInterruptMsgProc)
    {
		...
		if(nRelayingTime != 0)
		{
			if(GetSystemTimeInSeconds() - nRelayingTime > RelayingBlockMaxLimit)
			{
				LogPrint(BCLog::NET,"超过MaxLimit不出块，被遗弃\n");
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
	...
	//获取节点绑定的本地地址和端口，并生成对应的套接字，接受别的节点的请求。
	...
	//首先，生成套接字相关的线程，以便进行网络的接收和发送。处理方法和前面线程的类似
    threadSocketHandler = std::thread(&TraceThread<std::function<void()> >, "net", std::function<void()>(std::bind(&CConnman::ThreadSocketHandler, this)));
	//接下来，处理 DNS 种子节点线程，处理 DNS 种子相关的逻辑
