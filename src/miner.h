...
typedef boost::multi_index_container<
    ...
    boost::multi_index::indexed_by<
        boost::multi_index::ordered_unique<//ordered_unique 唯一索引 
            modifiedentry_iter,
            CompareCTxMemPoolIter
        >,
        // sorted by modified ancestor fee rate or gas price 按修改后的祖先费率或天然气价格分类
        boost::multi_index::ordered_non_unique<  //ordered_non_unique 非唯一索引   ，
            // Reuse same tag from CTxMemPool's similar index 从CTxMemPool的相似索引中重用相同的标记
...
/////////////////////////////////////////////
    ...
    CMutableTransaction originalRewardTx; // tsc
...
private:
	...
	// taoist ： addPackageTxs now return ticketNUM
    int addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated, uint64_t minGasPrice);
