    ...
    platformStyle(_platformStyle)
{
    ...
    tscVersionChecker = new TscVersionChecker(this);

    if(fCheckForUpdates && tscVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(TSC_RELEASES, TSC_RELEASES);
        QString message(tr("New version of Tsc wallet is available on the Tsc source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction = new QAction(platformStyle->MultiStatesIcon(":/icons/overview",PlatformStyle::NavBar,0x666666,0xffffff), tr("My &wallet"), this);
    ...
    sendCoinsAction = new QAction(platformStyle->MultiStatesIcon(":/icons/send_to",PlatformStyle::NavBar,0x666666,0xffffff), tr("&Send"), this);
    sendCoinsAction->setStatusTip(tr("Send coins to a Tsc address"));
    ...
    receiveCoinsAction = new QAction(platformStyle->MultiStatesIcon(":/icons/receive_from",PlatformStyle::NavBar,0x666666,0xffffff), tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tsc: URIs)"));
    ...
    smartContractAction = new QAction(platformStyle->MultiStatesIcon(":/icons/smart_contract",PlatformStyle::NavBar,0x666666,0xffffff), tr("Smart &Contracts"), this);
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history",PlatformStyle::NavBar,0x666666,0xffffff), tr("&Transactions"), this);
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken",PlatformStyle::NavBar,0x666666,0xffffff), tr("&TSC Tokens"), this);
    ...
    buyticketAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken",PlatformStyle::NavBar,0x666666,0xffffff), tr("&Buy Tickets"), this);
    buyticketAction->setStatusTip(tr("Buy Tickets (Buy tickets to join the ecosystem)"));
    buyticketAction->setToolTip(buyticketAction->statusTip());
    buyticketAction->setCheckable(true);
    buyticketAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_7));
    tabGroup->addAction(buyticketAction);
    ...
    connect(buyticketAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(buyticketAction, SIGNAL(triggered()), this, SLOT(gotoBuyTicketPage()));
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Tsc addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Tsc addresses"));
    ...
    openAction->setStatusTip(tr("Open a tsc: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Tsc command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(buyticketAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    buyticketAction->setEnabled(enabled);
}
...
void BitcoinGUI::gotoBuyTicketPage()
{
    buyticketAction->setChecked(true);
    if (walletFrame) walletFrame->gotoBuyTicketPage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Tsc network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Tsc"); // default title
