...
bool CBlockTreeDB::ReadTxIndex(const uint256 &txid, CDiskTxPos &pos) {
	LogPrint(BCLog::TICKETPOOL,"======ReadTxIndex======\n");
...
bool CBlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex)
{
	...
	LogPrint(BCLog::FINDBEGIN," pppppppppppppppppp\n");

    // Load mapBlockIndex
	//确认是否有效 void SeekToFirst(); 
    //从头开始找 void Next(); 
    //获取下一个元素 bool GetKey(K &key) 
    //获取key int GetKeySize() 
    //获取key的size bool GetValue(V &value) 
    //获取value int GetValueSize() //后去value的size
    while (pcursor->Valid()) {
		LogPrint(BCLog::FINDBEGIN," SSSSSSSSSS\n");

        boost::this_thread::interruption_point();
        std::pair<char, uint256> key;
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
			LogPrint(BCLog::FINDBEGIN," FFFFFFFFF\n");

            CDiskBlockIndex diskindex;
            if (pcursor->GetValue(diskindex)) {
				LogPrint(BCLog::FINDBEGIN," qqqqqqqqqqqqqqq\n");

                // Construct block index object
                CBlockIndex* pindexNew = insertBlockIndex(diskindex.GetBlockHash());
                pindexNew->pprev          = insertBlockIndex(diskindex.hashPrev);
                pindexNew->nHeight        = diskindex.nHeight;
				LogPrint(BCLog::FINDBEGIN," pindexNew->nHeight   :%d\n",pindexNew->nHeight);
                ...
                pindexNew->nMoneySupply   = diskindex.nMoneySupply; // TSC_INSERT_LINE
                pindexNew->nStatus        = diskindex.nStatus;
                pindexNew->nTx            = diskindex.nTx;
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // tsc
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // tsc
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
				pindexNew->seed           = diskindex.seed;
               // pindexNew->prevoutStake   = diskindex.prevoutStake;
               // pindexNew->vchBlockSig    = diskindex.vchBlockSig; // tsc
 ...
 /*               if (pindexNew->IsProofOfStake())
				...
				if (temp_height < pindexNew->nHeight)
				{
					temp_height = pindexNew->nHeight;
				}
                pcursor->Next();
            } else {
	...
	LogPrint(BCLog::FINDBEGIN,"temp_height : %d\n",temp_height);
    return true;
}
