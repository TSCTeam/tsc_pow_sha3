...
{
	  int WALLET_ENDPOINT_BASE_index = request.URI.find(WALLET_ENDPOINT_BASE);
      if(WALLET_ENDPOINT_BASE_index >= 0)
	  {
		 if (request.URI.substr(WALLET_ENDPOINT_BASE_index, WALLET_ENDPOINT_BASE.size()) == WALLET_ENDPOINT_BASE) {
		 // wallet endpoint was used
		 std::string firstStr = request.URI.substr(WALLET_ENDPOINT_BASE_index);
	     std::string requestedWallet = urlDecode(firstStr.substr(WALLET_ENDPOINT_BASE.size()));
		 LogPrint(BCLog::BUYTICKET,"requestedWallet :%s\n",requestedWallet);
		 for (CWalletRef pwallet : ::vpwallets) {
			LogPrint(BCLog::BUYTICKET,"requestedWallet 11111\n");
	  ...
	  LogPrint(BCLog::BUYTICKET,"requestedWallet from vpwallets[0] \n");
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Tsc address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account tsc address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 0)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Tsc address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tsc address to be associated with an account.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tsc address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a tsc address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static CAmount SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control, bool hasSender,bool getFee)
{
	...
	LogPrint(BCLog::COMMITTICKET,"sendtoaddress    :%s\n",wtxNew.tx->ToString());
...
//{
//	return CScript() << OP_BUYTICKET << OP_DUP << OP_HASH160 << ToByteVector(stakeaddress) << OP_EQUALVERIFY << OP_CHECKSIG;
//}
static CAmount toBuyTicketTx(CWallet * const pwallet, CAmount ticketprice, CAmount minerFee, CWalletTx& wtxNew, const CCoinControl& coin_control, bool fHasbuyTicketAddress)
{
	CAmount curBalance = pwallet->GetBalance();
	LogPrint(BCLog::BUYTICKET,"all Balance is : %jd how mach we need %jd\n",curBalance / 100000000, 1 +(ticketprice + minerFee) / 1000000000);

	//判断钱的时候 +1 coin 
	if (ticketprice + minerFee +1 * COIN > curBalance)
	{
		LogPrint(BCLog::BUYTICKET,"no enough money to buy ticket\n");
		throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "no enough money to buy ticket");
	}
	...
	if (!pwallet->CommitTicketTx(wtxNew, reservekey, g_connman.get(), state)) {
		//strError = strprintf("Error: The transaction was rejected! Reason given: %s", state.GetRejectReason());
	//	throw JSONRPCError(RPC_WALLET_ERROR, strError);
        std::cout <<"hcy_test: commit ticket failed:" <<wtxNew.tx->ToString() << " ,," << state.GetRejectReason() << std::endl;
	}
    else
    {
        
        std::cout <<"hcy_test: commit ticket ok:" <<wtxNew.tx->ToString() << std::endl;
    }
...
// ×¢²áµÄÂòÆ±½»Ò× [1/24/2019 taoist]
UniValue buyticket(const JSONRPCRequest& request)
{
	LogPrint(BCLog::BUYTICKET,"-----------------------------buyticket------------------------------------------------\n");

	// ÄÃµ½Ç®°ü¾ä±ú [1/24/2019 taoist]
	CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
	if (request.fHelp || request.params.size() > 3)
	{
		throw std::runtime_error(
			"buyticket \"miner_tip\" \"send_Address\" \"change_Address\" \n"
			"if don't have minerFee,minerFee is 1 tsc \n"
			"buyTicket  minerFee \"address\" \n"
			+ HelpRequiringPassphrase(pwallet) +
			"\nArguments:\n"
			"1. \"miner_tip\"            miner  packing charges \n"
			"2. \"send_Address\"            (string, required) user money from (address)\n"
			"3. \"change_Address\"          give change address \n"
			+ HelpExampleCli("buyticket","")
			+ HelpExampleCli("buyticket", "10 ")
			+ HelpExampleCli("buyticket", "10 \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \n")
			+ HelpExampleCli("buyticket", "10 \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\"  \"QMasdfpbz1BPpXFHz9m3CdqATR44Jvarew\"\n")
		);
	}
	//钱包可用
	if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
	...
	{
	    MinerFee = AmountFromValue(request.params[0]);
	    LogPrint(BCLog::ADDPACKAGETX,"miner fee is :%jd \n",MinerFee / 100000000);
	}
	else
	{
	    MinerFee = 1 * COIN;
	...
	LogPrint(BCLog::ADDPACKAGETX,"ticketprice is :%jd \n",ticketprice / 100000000);
	...
	{
		buyTicketAddress.SetString(request.params[1].get_str());
		LogPrint(BCLog::ADDPACKAGETX," buy ticket from  :%s\n",buyTicketAddress.ToString());

		if (!buyTicketAddress.IsValid())
			throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tsc address(buyTicketAddress) to send from");
		else
			fHasbuyTicketAddress = true;
	}
	else
	{
		LogPrint(BCLog::ADDPACKAGETX," don't have buyTicketAddress\n");
	}
	
	if (fHasbuyTicketAddress)
	{
		LogPrint(BCLog::ADDPACKAGETX,"HasbuyTicketAddress \n");
		...
		LogPrint(BCLog::ADDPACKAGETX,"wallet can be used :\n");
		for (const COutput& out : vecOutputs)
		{

			LogPrint(BCLog::ADDPACKAGETX,"            %s\n",out.ToString());

		}

		// coin_control Ìí¼ÓbuyTicketAddress
		int selectmoney = 0;
		for (const COutput& out : vecOutputs)
		{
			...
			selectmoney += (out.tx->tx->vout[out.i].nValue)/100000000;
			LogPrint(BCLog::ADDPACKAGETX,"***%s\n",out.ToString());
			coin_control.Select(COutPoint(out.tx->GetHash(), out.i));

		}
		//判断钱的时候 +1 coin 
		if (ticketprice/100000000+MinerFee/100000000+1>selectmoney)
		{

			LogPrint(BCLog::ADDPACKAGETX,"don't have enough selectmoney£º %jd  selectmoney %jd \n",ticketprice/100000000 + MinerFee/100000000 + 1,selectmoney);
			throw JSONRPCError(RPC_TYPE_ERROR, "don't have enough selectmoney");


		}
		LogPrint(BCLog::ADDPACKAGETX,"don't have enough selectmoney£º %jd    selectmoney  %jd\n",ticketprice / 100000000 + MinerFee / 100000000 + 1,selectmoney);
		LogPrint(BCLog::ADDPACKAGETX,"have enough money\n");
	...
	{
		LogPrint(BCLog::ADDPACKAGETX," don't have coin_control limit\n");
	}


	//Ä¬ÈÏÕÒÁã¸ø×Ô¼º£¬ÈôÊÇÓÐÖ¸¶¨µØÖ·´òµ½Ö¸¶¨µØÖ·
	...
	if (!fChangeToSender) {
		LogPrint(BCLog::ADDPACKAGETX," have change address \n");

		ChangeToSender.SetString(request.params[2].get_str());
		coin_control.destChange = ChangeToSender.Get();
	}
	else
	{
		//****************************************************
		LogPrint(BCLog::ADDPACKAGETX,"don't have change address \n");
		coin_control.destChange = buyTicketAddress.Get();
	}
	// Wallet comments
//	bool fSubtractFeeFromAmount = false;
	bool fGetTransactionFee = false;
	EnsureWalletIsUnlocked(pwallet);



	// ½»Ò×²úÉúµÄtx [1/24/2019 taoist]
	CWalletTx wtx;

	// ÕæµÄÂòÆ±Âß¼­ [1/24/2019 taoist]
	//							Ç®°ü      Æ±¼Û          ´ò°ü·Ñ    tx    Ó²±Ò¿ØÖÆ      Ö¸¶¨µØÖ·
	CAmount txFee = toBuyTicketTx(pwallet, ticketprice, MinerFee, wtx, coin_control, fHasbuyTicketAddress);
	if (fGetTransactionFee) {
		return ValueFromAmount(txFee);
	}
	LogPrint(BCLog::ADDPACKAGETX,"-----------------------------toBuyTicketTx------------------------------------------------\n");
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 11)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"address\"            (string, required) The tsc address to send to.\n"
            ...
            "                             The recipient will receive less tscs than you enter in the amount field.\n"
            ...
            "11. \"getTransactionFee\"	   (bool, optional, default=false) Return the transaction fee.\n"
            "\nResult:\n"
    ...
    senderAddress.SetString(request.params[8].get_str());
        if (!senderAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tsc address to send from");
        else
            fHasSender=true;
    }
	...
	if (request.params.size() > 10){
        fGetTransactionFee = request.params[10].get_bool();
	}
    ...
    txFee = SendMoney(pwallet, address.Get(), nAmount, fSubtractFeeFromAmount, wtx, coin_control, fHasSender,fGetTransactionFee);
	if (fGetTransactionFee){
		return ValueFromAmount(txFee);
	}
...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = tscDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(tscDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice TSC price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
...
UniValue sendtocontract(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Tsc price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
                "6. \"senderaddress\" (string, optional) The quantum address that will be used as sender.\n"
                "7. \"broadcast\" (bool, optional, default=true) Whether to broadcast the transaction or not.\n"
                "8. \"changeToSender\" (bool, optional, default=true) Return the change to the sender.\n"
                "9. \"getTransactionFee\"	   (bool, optional, default=false) Return the transaction fee.\n"
                "\nResult:\n"
                "[\n"
                "  {\n"
                "    \"txid\" : (string) The transaction id.\n"
	...
	if (request.params.size() > 8){
        fGetTransactionFee = request.params[8].get_bool();
	}
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The tsc address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tsc address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tsc address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a tsc address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The tsc address to send funds to.\n"
    ...
    SendMoney(pwallet, address.Get(), nAmount, false, wtx, no_coin_control, false,false);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The tsc address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less tscs than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Tsc address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Tsc address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of tsc addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) tsc address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A tsc address associated with the keys.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The tsc address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The tsc address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request_)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The tsc address involved in the transaction\n"
    ...
    if (!shouldWaitConf) {
        {
			//std::cout << "shouldWaitConf 111111111" << std::endl;
            LOCK2(cs_main, pwallet->cs_wallet);
            _wtx = &pwallet->mapWallet[hash];
        }

    } else {
		//std::cout << "shouldWaitConf 222222222" << std::endl;
        request.PollStart();
        while (true) {
            {
                LOCK2(cs_main, pwallet->cs_wallet);
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending tsc\n"
    ...
    StartShutdown();
    return "wallet encrypted; Tsc server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of tsc addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) tsc address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the tsc address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Tsc address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The tsc address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid tsc address");
...
UniValue getwork(const JSONRPCRequest& request)
{
	...
	if (request.fHelp || request.params.size() >= 2)
	{
		throw std::runtime_error(
			"\nArguments:\n"
			"1. address                tsc address\n"
			"used :	tsc-cli getwork\n"
			"used : tsc-cli getwork address \n"
	...
	UniValue result(UniValue::VOBJ);
    LogPrint(BCLog::MINER,"nRelayingBlock:%s\n",nRelayingBlock.ToString());
	if (!nRelayingBlock.IsNull())
	{
		result.push_back(Pair("miner_ready", false));
		return result;
	}
	...
	UniValue  strhex(realwork(coinbase_script, num_generate, max_tries, true));
	if (strhex.get_str().compare("don't have enough ticket")== 0)
	{
		result.push_back(Pair("miner_ready", false));
        result.push_back(Pair("strhex", strhex));
		return result;
	}
	else
	{
		result.push_back(Pair("miner_ready", true));
		result.push_back(Pair("strhex", strhex));
	}
	result.push_back(Pair("height", chainActive.Tip()->nHeight + 1));
	//result.push_back(Pair("strhex", realwork(coinbase_script, num_generate, max_tries, true)));
	return result;
}
...
{ //  category              name                        actor (function)           okSafeMode
	...
	{ "wallet",             "buyticket",				&buyticket,				   false,  { "tip","sendAddress","changeaddress"} },
