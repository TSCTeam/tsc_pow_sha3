...
bool CWalletTx::RelayWalletTransaction(CConnman* connman)
{
	...
	if (IsTicketTX())
	{
        LogPrint(BCLog::BUYTICKET,"is ticket tx\n");
		CValidationState state;
...
bool CWallet::CommitTicketTx(CWalletTx& wtxNew, CReserveKey& reservekey, CConnman* connman, CValidationState& state)
{
	LogPrint(BCLog::COMMITTICKET," -------------CommitTransaction start-------------------- \n");
	if (wtxNew.tx->vout[0].scriptPubKey.HasOpSStx())
	{
		if (wtxNew.tx->vout[0].nValue >= TICKET_PRICE * COIN)
		{
			LogPrintf("this ticket can  commit  ,ticket price enough ");
		}
		else
		{
			LogPrintf("can't commit ticket ,ticket price not enough : %d\n", wtxNew.tx->vout[0].nValue);
			return false;
		}	
	}
	else
	{
		LogPrintf("this  tx not is ticketTX    ");
		return false;
	}
	...
	LogPrint(BCLog::COMMITTICKET,"CommitTransaction: %s\n",wtxNew.tx->ToString());
	{
		...
		//添加TX到钱包，因为如果它有变化，它也是我们的，否则只是为了事务历史。
		bool fAddToWallet= AddToWallet(wtxNew);
		if (fAddToWallet)
		{
			LogPrint(BCLog::COMMITTICKET,"AddToWallet \n");
		}
		else
		{
			LogPrint(BCLog::COMMITTICKET,"AddToWallet error \n");
			//throw JSONRPCError(RPC_WALLET_ERROR, "AddToWallet error ");
			return false;
		}
		...
		LogPrint(BCLog::COMMITTICKET,"Notify that old coins are spent \n");
	}
	// Track how many getdata requests our transaction gets
	// 跟踪多少getData请求我们的事务获得
	mapRequestCount[wtxNew.GetHash()] = 0;
	bool dasdad = wtxNew.AcceptTicketTxToMemoryPool(maxTxFee, state);
	LogPrint(BCLog::COMMITTICKET,"-------------AcceptToMemoryPool end ------------------- %jd\n",maxTxFee);

	if (!dasdad) {
		LogPrint(BCLog::COMMITTICKET,"AcceptToMemoryPool not ok \n");

		LogPrintf("CommitTransaction(): Transaction cannot be broadcast immediately, %s\n", state.GetRejectReason());
		// TODO: if we expect the failure to be long term or permanent, instead delete wtx from the wallet and return failure.
		// 如果我们期望失败是长期的或永久的，则从钱包中删除wtx并返回失败。
	}
	else {
		LogPrint(BCLog::COMMITTICKET,"AcceptToMemoryPool ok \n");

		wtxNew.RelayWalletTransaction(connman);
	}
	LogPrint(BCLog::COMMITTICKET," -------------CommitTransaction end-------------------- \n");
...
bool CWallet::CreateTicketPurchaseTx(CWallet * const pwallet, CAmount ticketprice, CAmount minerFee, CWalletTx& wtxNew, const CCoinControl& coin_control, bool fHasbuyTicketAddress)                         
{
	// Parse Bitcoin address 
	// 打钱到目标地址
	std::vector<unsigned char> vOpTrue = { (unsigned char)OP_TRUE };
	uint160 p2shOpTrueAddr = Hash160(vOpTrue);
	CScript SSTXscriptPubKey = PayToSStx(p2shOpTrueAddr);
	if (SSTXscriptPubKey.HasOpSStx())
	{
		LogPrint(BCLog::BUYTICKET," HasOpSStx\n");
	}
	if (ticketprice < TICKET_PRICE * COIN)
	{
		LogPrint(BCLog::BUYTICKET,"TICKET_PRICE error \n");
		return false;
	}
	//特殊地址打钱完成，缺少找零
	CTxOut out(ticketprice, SSTXscriptPubKey);
	CMutableTransaction txNew;
	CFeeRate discard_rate = GetDiscardRate(::feeEstimator);
	txNew.vout.clear();
	txNew.vout.push_back(out);

	//获取可用的硬币 由coin_control提供
	std::vector<COutput>  vAvailableCoins;
	AvailableCoins(vAvailableCoins, true, &coin_control);
	LogPrint(BCLog::BUYTICKET,"AvailableCoins size :%d \n",vAvailableCoins.size());

	std::set<CInputCoin> setCoins;
	setCoins.clear();
	// setCoins 传出参数 被选中的硬币
	CAmount nValueIn = 0;
	LogPrint(BCLog::BUYTICKET,"ticketprice + minerFee:%jd, ticketprice + minerFee +1 :%jd\n",ticketprice + minerFee,ticketprice + minerFee +1 *COIN);
	if (!SelectCoins(vAvailableCoins, ticketprice + minerFee+1 * COIN, setCoins, nValueIn, &coin_control))
	{
		LogPrint(BCLog::BUYTICKET,"Insufficient funds \n");

		return false;
	}

	LogPrint(BCLog::BUYTICKET,"**nValueIn %jd\n",nValueIn);
	const CAmount nChange = nValueIn - ticketprice - minerFee;
	LogPrint(BCLog::BUYTICKET,"**nChange %jd\n",nChange);
	...
	if (nChange > 0)
	{
		...
		{
			LogPrint(BCLog::BUYTICKET, "****fHasbuyTicketAddress changeto destchange \n");
			scriptChange = GetScriptForDestination(coin_control.destChange);
		}
		else if (boost::get<CNoDestination>(&coin_control.destChange) && setCoins.size() > 0)
		{
			LogPrint(BCLog::BUYTICKET,"has coin_control.destChange ==>create scriptChange \n");

			auto pcoin = setCoins.begin();
			scriptChange = pcoin->txout.scriptPubKey;//找零脚本

		}
		CTxOut Changeout(nChange, scriptChange);
		if (IsDust(Changeout, discard_rate))
		{
			LogPrint(BCLog::BUYTICKET,"Changeout IsDust \n");
		...
		else
		{
			...
			LogPrint(BCLog::BUYTICKET,"ok \n");

		}
	}
	LogPrint(BCLog::BUYTICKET,"last txNew.vout___size :%d \n",txNew.vout.size());

	for (auto temp : txNew.vout)
	{
		LogPrint(BCLog::BUYTICKET,"last txNew.vout :%s \n",temp.ToString());
	}
	...
	if (fHasbuyTicketAddress && coin_control.HasSelected()) {
	...
	//LogPrint(BCLog::BUYTICKET,"last txNew.vin___size :" << txNew.vin.size() ;

	//for (auto temp : txNew.vin)
	//{
	//	LogPrint(BCLog::BUYTICKET,"last txNew.vin :" << temp.ToString() ;
	//}
	// Fill in dummy signatures for fee calculation.
	// 填写虚拟签名以计算费用。
	if (!DummySignTx(txNew, vCoins)) {
		LogPrint(BCLog::BUYTICKET,"DummySignTx\n");

		return false;
	}

	// Remove scriptSigs to eliminate the fee calculation dummy signatures
	// 删除ScriptSigs以消除费用计算虚拟签名
	for (auto& vin : txNew.vin) {
		vin.scriptSig = CScript();
		vin.scriptWitness.SetNull();
	}
	LogPrint(BCLog::BUYTICKET,"last txNew.vin___size222 :%d \n",txNew.vin.size());

	for (auto temp : txNew.vin)
	{
		LogPrint(BCLog::BUYTICKET,"last txNew.vin2 :%s \n",temp.ToString());
	}
	//签名
	if (true)
	{
		...
		LogPrint(BCLog::BUYTICKET," signed vin   \n");
		for (auto vin : txNewConst.vin)
		{
			LogPrint(BCLog::BUYTICKET,"   %s ",vin.ToString());
		}
		LogPrint(BCLog::BUYTICKET," signed vout   \n");
		for (auto vout : txNewConst.vout)
		{
			LogPrint(BCLog::BUYTICKET,"  vout :  %s\n",vout.ToString());
		}
	}
	...
	LogPrint(BCLog::BUYTICKET," is ok\n");
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
				...
				LogPrint(BCLog::BUYTICKET,"last txNew.vin___size : %d\n",txNew.vin.size());

				for (auto temp : txNew.vin)
				{
					LogPrint(BCLog::BUYTICKET,"last txNew.vin3 :%s \n",temp.ToString());
				}
...
bool CMerkleTx::AcceptTicketTxToMemoryPool(const CAmount& nAbsurdFee, CValidationState& state)
{
	return ::AcceptTicketTxToMemoryPool(mempool, state, tx, true, nullptr, nullptr, false, nAbsurdFee);
	//return true;
}
