...
/////////////////////////////////////////// tsc
#include <tsc/tscstate.h>
#include <tsc/tscDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <tsc/storageresults.h>
...
using ExtractTscTX = std::pair<std::vector<TscTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // tsc
...
extern CBlock nRelayingBlock;
extern int64_t nRelayingTime;
extern uint32_t nNextChoice;


extern uint256 nDownLoadingRelayingHash;
extern ticketPool ticketpool;
extern luckyTicket luckyticket;
...
//get ticket 
bool GetTicketTransaction(const uint256 &hash, CTransactionRef &tx, const Consensus::Params& params, uint256 &hashBlock, bool fAllowSlow = false);
...
bool checkBlockTicket(const CBlock& block);
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashTscRoot,
    const dev::h256& oldHashStateRoot, const std::vector<TscTransaction>& transactions);
...
class TscTxConverter{

public:

    TscTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionTscTransactions(ExtractTscTX& tscTx);
...
private:
    ...
    TscTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<TscTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
