...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // tsc
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // tsc
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    UniValue result(UniValue::VOBJ);
    result.push_back(Pair("hash", blockindex->GetBlockHash().GetHex()));
	result.push_back(Pair("seed",  (int)blockindex->seed));
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // tsc
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // tsc
	...
	for (const auto& tx : block.vtx)
	{
		if (txDetails)
		{
			 bool isticket = (tx->vout[0]).scriptPubKey.HasOpSStx();
			if (isticket)
			{

				UniValue objTx(UniValue::VOBJ);
				TxToUniv(*tx, uint256(), objTx, true, RPCSerializationFlags());
				tickets.push_back(objTx);
			}
			else
			{
		...
		else
		{
			...
			if (isticket)
			{
				...
				tickets.push_back(tx->GetHash().GetHex());
			}
			else
			{
	...
	result.push_back(Pair("tickets", tickets));
...
UniValue ticketPoolToJSON()
{
	UniValue ret(UniValue::VOBJ);
	LogPrint(BCLog::RPC,"**********************\n");

	ticketpool.print_ticket();
	LogPrint(BCLog::RPC,"**********************\n");

	ret.push_back(Pair("ticketpoolSize", (int64_t)ticketpool.ticket_pool.size()));
	ret.push_back(Pair("loadHeight", (int64_t)ticketpool.getLoadHeight()));
	LogPrint(BCLog::RPC,"1.1\n");

	//////////////////////////
	if (chainActive.Height() < TICKET_VALVE+1)
	{
		LogPrint(BCLog::RPC,"1.2\n");

		ret.push_back(Pair("luckyticket", "don't have start ticket "));

	}
	else
	{
		LogPrint(BCLog::RPC,"1.3\n");

		std::vector<unsigned int > luckyIndex = luckyticket.getLuckyIndex(chainActive.Height());
		std::vector<CBitcoinAddress> vluckyaddress;
		LogPrint(BCLog::RPC, "1.4\n");

		UniValue VTicket(UniValue::VARR);
		...
		for (auto index : luckyIndex)
		{
			...
			std::string str(lucky_ticket.ticketAddress.ToString());
			str += "    ";
			char str1[20] = { 0 };
			sprintf(str1, "%lu", lucky_ticket.ticketPrice);

			str += std::string (str1);
			str += "    ";
			char str2[9] = { 0 };
			sprintf(str2, "%d", lucky_ticket.ticketHeight);
			LogPrint(BCLog::RPC,"1.5\n");

			str += std::string(str2);

			str += "    ";
			str += lucky_ticket.ticketTxHash.ToString();
			char strindex[9] = { 0 };
			sprintf(strindex, "%d", index);
			LogPrint(BCLog::RPC,"1.6\n");

			ticket.pushKV(strindex, str);
			VTicket.push_back(ticket);
		}


		ret.pushKV("luckyTicket", VTicket);
...
UniValue getticketpoolinfo(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() != 0)
		throw std::runtime_error(
			"getticketpoolinfo\n"
			"\nReturns the information of the ticketpool.\n"
			"\nResult:string"
			"n    information of the ticketpool\n"
			"\nExamples:\n"
			+ HelpExampleCli("getticketpoolinfo", "")
			+ HelpExampleRpc("getticketpoolinfo", "")
		);

	return ticketPoolToJSON();
	//return 111;
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress tscSenderAddress(request.params[2].get_str());
        if(tscSenderAddress.IsValid()){
            CKeyID keyid;
            tscSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
	...
	{ "blockchain",         "getticketpoolinfo",      &getticketpoolinfo,      true,{} },
