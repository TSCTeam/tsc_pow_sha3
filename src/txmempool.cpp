...
// 计算未在setDescendants中的条目的后代，并将其添加到setDescendants中。假设entryit已经是内存池中的TX，setMemPoolChildren对TX和所有后代都是正确的
// Also assumes that if an entry is in setDescendants already, then all
// in-mempool descendants of it are already in setDescendants as well, so that we
// can save time by not iterating over those entries.
// 还假设如果条目已经在setDescendants中，那么它的所有模池后代也都在setDescendants中，这样我们就可以通过不对这些条目进行迭代来节省时间。
void CTxMemPool::CalculateDescendants(txiter entryit, setEntries &setDescendants)
{
	...
	// 遍历条目的子元素，只添加在setDescendants中没有考虑到的子元素(因为这些子元素已经被遍历，或者将在这个迭代中被遍历)。
    while (!stage.empty()) {
