...
#if defined(NDEBUG)
# error "Tsc cannot be compiled without assertions."
#endif
...
std::unique_ptr<TscState> globalState;
...
CBlock nRelayingBlock;
int64_t nRelayingTime = 0;
uint32_t nNextChoice = 0;
bool floadok = false;

uint256 nDownLoadingRelayingHash;
...
const std::string strMessageMagic = "Tsc Signed Message:\n";
...
static bool AcceptTickeTxToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
	bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
	bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
	LogPrint(BCLog::COMMITTICKET,"AcceptTickeTxToMemoryPoolWorker %jd\n",maxTxFee);

	const CTransaction& tx = *ptx;
	LogPrint(BCLog::COMMITTICKET,"@@@@@@@@    :  %s\n", tx.ToString().c_str());
	const uint256 hash = tx.GetHash();
	if (!CheckTransaction(tx, state))
	{
		LogPrint(BCLog::COMMITTICKET, "CheckTransaction error  \n");
		return false; // state filled in by CheckTransaction
	}
	LogPrint(BCLog::COMMITTICKET, "CheckTransaction ok \n");

	// Coinbase is only valid in a block, not as a loose transaction
	if (tx.IsCoinBase())
	{
		LogPrint(BCLog::COMMITTICKET, "Coinbase is only valid in a block, not as a loose transaction \n");
	...
	if (tx.IsCoinStake())
	{
		LogPrint(BCLog::COMMITTICKET, "coinstake is only valid in a block, not as a loose transaction  \n");
		return state.DoS(100, false, REJECT_INVALID, "coinstake");
	}
	LogPrint(BCLog::COMMITTICKET, "CheckTransaction isn't coinstake and  IsCoinBase \n");
	...
	bool fHasOpSStx = false;
	for (auto out : tx.vout)
	{
		if (out.scriptPubKey.HasOpSStx())
		{
			fHasOpSStx = true;
			LogPrint(BCLog::COMMITTICKET, "HasOpSStx :this tx is ticketTx\n");
		}
	}

	std::string reason;
	if (!fHasOpSStx && fRequireStandard && !IsStandardTx(tx, reason, witnessEnabled))
	{
		LogPrint(BCLog::COMMITTICKET, " Rather not work on nonstandard transactions (unless -testnet/-regtest) error  \n");
	...
	// be mined yet.
	// 只接受nLockTime-使用可以在下一个块中挖掘的事务；我们不希望我们的内存池中满是无法挖掘的事务。
	if (!CheckFinalTx(tx, STANDARD_LOCKTIME_VERIFY_FLAGS))
	{
		LogPrint(BCLog::COMMITTICKET, "CheckFinalTx error  \n");
	...
	if (pool.exists(hash))
	{
		LogPrint(BCLog::COMMITTICKET, " pool exists transaction \n");

		return state.Invalid(false, REJECT_DUPLICATE, "txn-already-in-mempool");
	}
	else
	{
		LogPrint(BCLog::COMMITTICKET, " pool  NOT exists transaction ==> this Tx can be insert to mempool\n");
	}
			...
			if (itConflicting != pool.mapNextTx.end())
			{
				...
				if (!setConflicts.count(ptxConflicting->GetHash()))
				{
					...
					if (fReplacementOptOut) {
						LogPrint(BCLog::COMMITTICKET, "fReplacementOptOut ！\n");
	...
	LogPrint(BCLog::COMMITTICKET, "Check for conflicts with in-memory transactions : OK \n");
		...
		LockPoints lp;
		{
			...
			for (size_t out = 0; out < tx.vout.size(); out++)
			{
				...
				if (view.HaveCoin(outpoint))
				{
					...
					LogPrint(BCLog::COMMITTICKET, "txn-already-known\n");

					return state.Invalid(false, REJECT_DUPLICATE, "txn-already-known");
				}
			}
			LogPrint(BCLog::COMMITTICKET, "do we already have it :don't have \n");
			// do all inputs exist? 判断vin存在
			for (const CTxIn txin : tx.vin)
			{
				...
				if (!view.HaveCoin(txin.prevout))
				{
					...
					// 否则假设这可能是我们还没见过父母的孤儿TX
					if (pfMissingInputs)
					{
						*pfMissingInputs = true;
					}
					LogPrint(BCLog::COMMITTICKET, "fMissingInputs and !state.IsInvalid() is used to detect this condition, don't set state.Invalid()\n");
			...
			LogPrint(BCLog::COMMITTICKET, "XXXXXXXXXXXXXX  nValueIn %jd\n",nValueIn);
			...
			bool bbbbbbb = CheckSequenceLocks(tx, STANDARD_LOCKTIME_VERIFY_FLAGS, &lp);
			//std::string sdaas = bbbbbbb ? "true" : "false";
			//LogPrint(BCLog::COMMITTICKET, "bbbbb : " << sdaas ;
			if (!bbbbbbb)
			{
				LogPrint(BCLog::COMMITTICKET, "non-BIP68-final\n");

				return state.DoS(0, false, REJECT_NONSTANDARD, "non-BIP68-final");
			}

			//LogPrint(BCLog::COMMITTICKET, "777777777777777777 " ;

			// Check for non-standard pay-to-script-hash in inputs
			if (fRequireStandard && !AreInputsStandard(tx, view))
			{
				LogPrint(BCLog::COMMITTICKET, "bad-txns-nonstandard-inputs\n");
			...
			{
				LogPrint(BCLog::COMMITTICKET, "bad-witness-nonstandard\n");

				return state.DoS(0, false, REJECT_NONSTANDARD, "bad-witness-nonstandard", true);
			}
			//LogPrint(BCLog::COMMITTICKET, "88888888888888888888888888888 " ;
			...
			//跟踪花费Coinbase的事务，我们在Reorgs期间重新扫描这些事务，以确保Coinbase_成熟度仍然满足。
			...
			//检查sigop 大小 dgpMaxTxSigOps==16000
			if (nSigOpsCost > dgpMaxTxSigOps)
			{
				LogPrint(BCLog::COMMITTICKET, "bad-txns-too-many-sigops\n");
			...
			{
				LogPrint(BCLog::COMMITTICKET, "mempool min fee not met\n");

				return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "mempool min fee not met", false, strprintf("%d < %d", nFees, mempoolRejectFee));
			}
			//LogPrint(BCLog::COMMITTICKET, "00000000000000000000000000 " ;


			// No transactions are allowed below minRelayTxFee except from disconnected blocks
			// 除断开连接的块外，minRelayTxFee以下不允许任何事务。
			if (fLimitFree && nModifiedFees < ::minRelayTxFee.GetFee(nSize)) {
				LogPrint(BCLog::COMMITTICKET, "min relay fee not met\n");

				return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "min relay fee not met");
			}

			//过高的交易费 但是买票是允许的
			if (!fHasOpSStx && !tx.HasCreateOrCall() && nAbsurdFee && nFees > nAbsurdFee)
			{
				//LogPrint(BCLog::COMMITTICKET, "absurdly-high-11111111: " << nAbsurdFee;

				//LogPrint(BCLog::COMMITTICKET, "absurdly-high-fee : " << nFees;

				//LogPrint(BCLog::COMMITTICKET, "absurdly-high-fee" ;

				return state.Invalid(false,
					REJECT_HIGHFEE, "absurdly-high-fee",
					strprintf("%d > %d", nFees, nAbsurdFee));
			}
			//LogPrint(BCLog::COMMITTICKET, "absurdly-high-fee pass" ;
			...
			if (!pool.CalculateMemPoolAncestors(entry, setAncestors, nLimitAncestors, nLimitAncestorSize, nLimitDescendants, nLimitDescendantSize, errString)) {
				LogPrint(BCLog::COMMITTICKET, "too-long-mempool-chain\n");
			...
			// intersect.
			// 使用将被其替换的输出的事务是无效的。现在我们有了所有祖先的集合，我们可以通过确保setConflicts和setAncestors不相交来检测这个病态病例。
			for (CTxMemPool::txiter ancestorIt : setAncestors)
			{
				const uint256 &hashAncestor = ancestorIt->GetTx().GetHash();
				if (setConflicts.count(hashAncestor))
				{
					LogPrint(BCLog::COMMITTICKET, "bad-txns-spends-conflicting-tx\n");
			...
			// 如果我们不保存锁，那么所有的冲突可能是不完整的；随后的RemoveStage()和addUncheck()调用不能保证内存池的一致性。
			LOCK(pool.cs);
			const bool fReplacementTransaction = setConflicts.size();
			if (fReplacementTransaction)
			{
				LogPrint(BCLog::COMMITTICKET, "fReplacementTransaction\n");
				...
				for (const uint256 &hashConflicting : setConflicts)
				{
					...
					{
						LogPrint(BCLog::COMMITTICKET, "insufficient fee\n");
				...
				if (nConflictingCount <= maxDescendantsToVisit) {
					...
					LogPrint(BCLog::COMMITTICKET, "too many potential replacements\n");
				...
				for (unsigned int j = 0; j < tx.vin.size(); j++)
				{
					...
					if (!setConflictsParents.count(tx.vin[j].prevout.hash))
					{
						...
						{
							LogPrint(BCLog::COMMITTICKET, "replacement-adds-unconfirmed\n");
				...
				{
					LogPrint(BCLog::COMMITTICKET, "insufficient fee 2\n");
			...
			LogPrint(BCLog::COMMITTICKET, "NO fReplacementTransaction\n");
			...
			//检查以前的事务，这是最后完成的，以帮助防止CPU耗尽、拒绝服务攻击。
			...
			if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, false, txdata))
			{
				...
				LogPrint(BCLog::COMMITTICKET, "CheckInputs \n");
			...
			// 再次检查当前块提示的脚本验证标志，以缓存脚本执行标志。当然，如果下一个块
			// 有与前一个块不同的脚本标志，这当然是无用的，但是由于缓存跟踪我们的脚本标志，
			// 它将自动失效，我们只会在软分叉激活上有几个额外的错误块。
			//
			...
			//
			// 这对于标准标志中的bug也很有用，当事务实际上无效时，这些错误会导致事务传递为有效。
			// 例如，STRICTENC标志错误地允许某些CHECKSIG非脚本传递，尽管它们是无效的。
			//
			// There is a similar check in CreateNewBlock() to prevent creating
			// invalid blocks (using TestBlockValidity), however allowing such
			// transactions into the mempool can be exploited as a DoS attack.
			// CreateNewBlock()中也有类似的检查，以防止创建无效的块(使用TestBlockVality)，
			// 但是，允许这样的事务进入内存池可能会被用作DoS攻击。
			unsigned int currentBlockScriptVerifyFlags = GetBlockScriptFlags(chainActive.Tip(), Params().GetConsensus());


			if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata))
			{
				LogPrint(BCLog::COMMITTICKET, "CheckInputsFromMempoolAndCache\n");
				...
				{
					LogPrint(BCLog::COMMITTICKET, "BUG! PLEASE REPORT THIS! ConnectInputs failed against latest-block but not STANDARD flags\n");
				...
				else
				{
					...
					{
						LogPrint(BCLog::COMMITTICKET, "ConnectInputs failed against MANDATORY but not STANDARD flags due to promiscuous mempool\n");
			...
			LogPrint(BCLog::COMMITTICKET, "OK CheckInputsFromMempoolAndCache\n");
			...
			// Store transaction in memory 将事务存储在内存中
			pool.addUnchecked(hash, entry, setAncestors, validForFeeEstimation);

			// trim mempool and check if tx was trimmed 修剪内存池并检查是否修剪了tx
			if (!fOverrideMempoolLimit) {
				LimitMempoolSize(pool, gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000, gArgs.GetArg("-mempoolexpiry", DEFAULT_MEMPOOL_EXPIRY) * 60 * 60);
				if (!pool.exists(hash))
				{
					LogPrint(BCLog::COMMITTICKET, "!pool.exists(hash)\n");
	...
	LogPrint(BCLog::COMMITTICKET, "commit ok\n");
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = tscDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = tscDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            TscTxConverter converter(tx, NULL);
            ExtractTscTX resultConverter;
            if(!converter.extractionTscTransactions(resultConverter)){
            ...
            std::vector<EthTransactionParams> tscETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(TscTransaction tscTransaction : tscTransactions){
                sumGas += tscTransaction.gas() * tscTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, tscTransaction.gasPrice());
                } else {
                    txMinGasPrice = tscTransaction.gasPrice();
                }
                VersionVM v = tscTransaction.getVersion();
                if(v.format!=0)
                ...
                if(tscTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(tscTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(tscTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)tscTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(tscETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > tscTransactions.size())
    ...
    GetMainSignals().TransactionAddedToMempool(ptx);
	LogPrint(BCLog::COMMITTICKET, "OK COMMIT\n");
...
static bool AcceptTicketTxToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
	bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
	bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool rawTx = false)
{
	std::vector<COutPoint> coins_to_uncache;
	bool res = AcceptTickeTxToMemoryPoolWorker(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache, rawTx);
	if (!res) {
		LogPrint(BCLog::COMMITTICKET, "AcceptTickeTxToMemoryPoolWorker error\n");
		return res;
	}
...
bool AcceptTicketTxToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
								bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced,
								bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool rawTx)
{
	const CChainParams& chainparams = Params();

	bool fAcceptTicketTxToMemoryPoolWithTime =AcceptTicketTxToMemoryPoolWithTime(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, rawTx);
	return fAcceptTicketTxToMemoryPoolWithTime;
}
bool GetTicketTransaction(const uint256 &hash, CTransactionRef &txOut, const Consensus::Params& consensusParams, uint256 &hashBlock, bool fAllowSlow)
{


	//find from rawtransaction
	...
	if (fAllowSlow) { // use coin database to locate block that contains transaction, and scan it
        ...
        LogPrint(BCLog::TICKETPOOL," coin.out.ToString() = %s\n",coin.out.ToString());
		LogPrint(BCLog::TICKETPOOL,"coin.nHeight = %d\n",coin.nHeight);
	...
	if (pindexSlow)
	{
		LogPrint(BCLog::TICKETPOOL," yes pindexSlow \n");
	}
	else
	{
		LogPrint(BCLog::TICKETPOOL," no  pindexSlow \n");
	}


	if (pindexSlow) {
		CBlock block;
		LogPrint(BCLog::TICKETPOOL,"hash = %s\n",hash.ToString());

		if (ReadBlockFromDisk(block, pindexSlow, consensusParams)) {
			LogPrint(BCLog::TICKETPOOL,"ReadBlockFromDisk OK\n");
			for (const auto& tx : block.vtx) {
				LogPrint(BCLog::TICKETPOOL, "%s\n", tx->ToString());
				if (tx->GetHash() == hash) {

					txOut = tx;
					hashBlock = pindexSlow->GetBlockHash();
					LogPrint(BCLog::TICKETPOOL,"HASH BLOCK  OK = %s\n",hashBlock.ToString());
	...
	{
		LogPrint(BCLog::TICKETPOOL," 1111111111111111111111111\n");
		CDiskTxPos postx;
		if (pblocktree->ReadTxIndex(hash, postx)) {
			...
			{
				LogPrint(BCLog::TICKETPOOL,"txOut->GetHash() != hash \n");
				return error("%s: txid mismatch", __func__);
			}
			LogPrint(BCLog::TICKETPOOL," =========find tickettx on disk ===============\n");
			return true;
		}
		LogPrint(BCLog::TICKETPOOL," fTxIndex \n");
	}

	//find from transaction
	{
		...
		LogPrint(BCLog::TICKETPOOL,"in find buy ticket address Wallet Is Available \n");
		
		{
			LOCK2(cs_main, pwallet->cs_wallet);
			if (!pwallet->mapWallet.count(hash))
			{
				LogPrint(BCLog::TICKETPOOL,"Invalid or non-wallet transaction id\n");
				return false;
			}
		...
		LogPrint(BCLog::TICKETPOOL,"hashblock 11 = %s\n",hashBlock.ToString());
		txOut = wtx.tx;
		return true;
	}
	return false;
...
bool GetTransaction(const uint256 &hash, CTransactionRef &txOut, const Consensus::Params& consensusParams, uint256 &hashBlock, bool fAllowSlow)
{
	...
	//LogPrint(BCLog::TICKETPOOL,"-------------begin---------------- ";
	...
	LogPrint(BCLog::TICKETPOOL," ---------last--------------- \n");
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    /*if(nHeight <= consensusParams.nLastPOWBlock)
        return 20000 * 13 * COIN;*/
	if(nHeight <= TICKET_VALVE)
        return 260000 * COIN;
    ...
    CAmount nSubsidy = 148 * COIN;//148 230
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // tsc
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // tsc
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    TscTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<TscTransaction>(1, callTransaction), blockGasLimit);
...
bool CheckReward(const CBlock& block, CValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, CAmount nActualStakeReward, const std::vector<CTxOut>& vouts)
{
    ...
    if (block.IsProofOfWork())
    {
		...
		{
			//LogPrint(BCLog::TICKETPOOL,"delete the most old ticket from ticketpool 33";
			if (tx->IsCoinBase())
			{
				//LogPrint(BCLog::TICKETPOOL,"height size " << vHticket.size();
				N = tx->vout.size()-1;
		
				break;
			}
		}
        CAmount blockReward = nFees + GetBlockSubsidy(nHeight, consensusParams)+ N* TICKET_PRICE * COIN;
		//if (block.vtx[offset]->GetValueOut() > blockReward)
		//std::cout<<"real out"<<block.vtx[offset]->GetValueOut()<<std::endl;
        if (block.vtx[offset]->GetValueOut() > blockReward)
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path tscDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(tscDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(tscDir.string(), std::ios::in | std::ios::out);
...
bool TscTxConverter::extractionTscTransactions(ExtractTscTX& tsctx){
    std::vector<TscTransaction> resultTX;
    ...
    tsctx = std::make_pair(resultTX, resultETP);
    return true;
}

bool TscTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool TscTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
TscTransaction TscTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    TscTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = TscTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = TscTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    ///////////////////////////////////////////////// // tsc
    TscDGP tscDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setTscSchedule(tscDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = tscDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = tscDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = tscDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // tsc
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if(tx.IsCoinBase()){
            ...
            if(pindex->nHeight > TICKET_VALVE)
            {
				nValueOut += tx.GetCoinBaseRealOut() - TICKET_PRICE * 5 * COIN;
				
			}
			else
			{
				nValueOut += tx.GetCoinBaseRealOut();
...
///////////////////////////////////////////////////////////////////////////////////////// tsc
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            TscTxConverter convert(tx, &view, &block.vtx);

            ExtractTscTX resultConvertTscTX;
            if(!convert.extractionTscTransactions(resultConvertTscTX)){
            ...
            if(!CheckMinGasPrice(resultConvertTscTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertTscTX.first, blockGasLimit);
            ...
            for(TscTransaction& qtx : resultConvertTscTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertTscTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertTscTX.first[k].from(), resultConvertTscTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
	...
	//std::cout<<"nMoneySupply "<< pindex->nMoneySupply / COIN<< " nValueOut "<<nValueOut/ COIN<<" nValueIn "<<nValueIn/ COIN<<" pindex->nHeight "<<pindex->nHeight<<std::endl; 
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
       ...
       // if(pindex->nMoneySupply > (uint64_t)(100000000 * 13 + ((pindex->nHeight - 5000) * 148)) * COIN){
       if(pindex->nMoneySupply > (uint64_t)((100000000 * 13 + (pindex->nHeight - TICKET_VALVE) * 2200)) * COIN){
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
	...
	if (pindexNew->nHeight > TICKETPOOL_HEIGHT_LATENCY && floadok)
	{
		LogPrint(BCLog::TICKETPOOL,"======update TIP======\n");
        /*
        LogPrint(BCLog::TICKETPOOL,"ticketpool.getLoadHeight() = %d ,pindexNew->nHeight = %d ,[UpdateTip]ticketpool.size1 = %d\n",ticketpool.getLoadHeight(),pindexNew->nHeight,ticketpool.ticket_pool.size());
        for (auto i = ticketpool.getLoadHeight(); i < pindexNew->nHeight - 1; i++ )
        {
            updateTicketpool(chainActive[i], chainActive[i]->nHeight);
        }
        */
        LogPrint(BCLog::TICKETPOOL,"[UpdateTip]ticketpool.size2 = %d\n",ticketpool.ticket_pool.size());
		updateTicketpool(pindexNew, pindexNew->nHeight);
        LogPrint(BCLog::TICKETPOOL,"[UpdateTip]ticketpool.size3 = %d\n",ticketpool.ticket_pool.size());
	}
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // tsc
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // tsc
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // tsc
            globalState->setRootUTXO(oldHashUTXORoot); // tsc
 ...
 * 返回链中工作最多的部分，这是不知道是无效的(然而，它远不能肯定是有效的)。
 */
 ...
 * *尝试在使pindexMostWork成为活动块方面取得一些进展。p块要么是nullptr，要么是指向对应于pindexMostWork的CBlock的指针。
 * 激活最佳链步
 */
 ...
 * 使最好的链活动，在多个步骤。其结果要么是失败，要么是激活的最佳链。pblock要么是nullptr，要么是指向已经加载的块的指针(以避免再次从磁盘加载)。
 */
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
	...
	//注意，虽然我们经常从ProcessNewBlock被叫到这里，但这远不是一个保证。P2P/RPC中的东西通常会在ProcessNewBlock中间调用我们-不要以为pBlock是为性能或正确性而设置的！
    CBlockIndex *pindexMostWork = nullptr;
    CBlockIndex *pindexNewTip = nullptr;
    int nStopAtHeight = gArgs.GetArg("-stopatheight", DEFAULT_STOPATHEIGHT);
    do {
        boost::this_thread::interruption_point();//线程执行到这个函数所在的语句就可以被中断
        if (ShutdownRequested())//是否中断请求
            break;

        const CBlockIndex *pindexFork;
        bool fInitialDownload;//初步下载
        {
            LOCK(cs_main);
            ConnectTrace connectTrace(mempool); // Destructed before cs_main is unlocked 在cs_main未锁定之前销毁
            CBlockIndex *pindexOldTip = chainActive.Tip();
            if (pindexMostWork == nullptr) {
                pindexMostWork = FindMostWorkChain();
            }

            // Whether we have anything to do at all. 不管我们有什么事要做。
            if (pindexMostWork == nullptr || pindexMostWork == chainActive.Tip())
                return true;

            bool fInvalidFound = false;//发现是否有效
...
static CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
  ...
  /*  if (pindexNew->IsProofOfStake())
	...
	// pindexNew->nStakeModifier = ComputeStakeModifier(pindexNew->pprev, block.IsProofOfWork() ? hash : block.prevoutStake.hash);
	 pindexNew->nStakeModifier = ComputeStakeModifier(pindexNew->pprev,  hash );
...
bool CheckBlockSignature(const CBlock& block)
{
	...
	// 区块头已删除vchblocksig [12/13/2018 taoist]
	/*std::vector<unsigned char> vchPubKey;
	if (!GetBlockPublicKey(block, vchPubKey))
	{
		return false;
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.fChecked)
        return true;
	//LogPrint(BCLog::TICKETPOOL,"ret 1";
    // Check that the header is valid (particularly PoW).  This is mostly
    // redundant with the call in AcceptBlockHeader.
    //检验区块头是否有效，特别是工作量证明
    if (!CheckBlockHeader(block, state, consensusParams, fCheckPOW))
        return false;
	//LogPrint(BCLog::TICKETPOOL,"ret 2";
	// 去除pos检查 [12/6/2018 taoist]
    ...
    if (fCheckMerkleRoot) {
		...
		//LogPrint(BCLog::TICKETPOOL,"ret 3";
        // Check for merkle tree malleability (CVE-2012-2459): repeating sequences
        // of transactions in a block without affecting the merkle root of a block,
        // while still invalidating it.
        if (mutated)
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-duplicate", true, "duplicate transaction");
		//LogPrint(BCLog::TICKETPOOL,"ret 4";
    }
	...
	//  Size limits [12/6/2018 taoist]
	if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // tsc
		return state.DoS(100, false, REJECT_INVALID, "bad-blk-length", false, "size limits failed");
	//LogPrint(BCLog::TICKETPOOL,"ret 5";
	// 编译错误 btc 和 tsc检查不一样 使用上面的  [12/7/2018 taoist]
	//if (block.vtx.empty() || block.vtx.size() * WITNESS_SCALE_FACTOR > dgpMaxBlockWeight || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT)
	//	return state.DoS(100, false, REJECT_INVALID, "bad-blk-length", false, "size limits failed");
	...
	//LogPrint(BCLog::TICKETPOOL,"ret 6";
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinBase())
            return state.DoS(100, false, REJECT_INVALID, "bad-cb-multiple", false, "more than one coinbase");

	//LogPrint(BCLog::TICKETPOOL,"ret 7";
	...
	//LogPrint(BCLog::TICKETPOOL,"ret 8";
	// 不会再有chinstake [12/6/2018 taoist]
   ...
   /* if (fCheckSig && !CheckBlockSignature(block))
    ...
    // Check transactions
	//LogPrint(BCLog::TICKETPOOL,"ret 8.5";
    for (const auto& tx : block.vtx) {
		if (!CheckTransaction(*tx, state, true))
		{
			//LogPrint(BCLog::TICKETPOOL,"ret 8.1";
        ...
        if(tx->HasOpSpend()){
			//LogPrint(BCLog::TICKETPOOL,"ret 8.3";
            if(!lastWasContract){
				//LogPrint(BCLog::TICKETPOOL,"ret 8.2";
		...
		//LogPrint(BCLog::TICKETPOOL,"ret 8.4";
        lastWasContract = tx->HasCreateOrCall() || tx->HasOpSpend();
    }

	//LogPrint(BCLog::TICKETPOOL,"ret 9";
	...
	//LogPrint(BCLog::TICKETPOOL,"ret 10";
    if (fCheckPOW && fCheckMerkleRoot)
        block.fChecked = true;
	//LogPrint(BCLog::TICKETPOOL,"ret 11";
    return true;
}
...
在磁盘上存储块。如果dbp是非nullptr，则已知该文件已驻留在磁盘上。*/
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
	...
	{
		LogPrint(BCLog::COMMITTICKET, "addtoblock\n");
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
	...
	CBlockIndex *pindex = nullptr;
	{
		if (chainActive.Height() >= TICKET_VALVE)
		{
			...
			luckyindex = luckyticket.getLuckyIndex(chainActive.Height(), pblock->seed);                                                                                                           

			int i = 1;
			CBlock temp_block = *pblock;
			...
			for (auto index : luckyindex)
			{
				...
				if (CheckSigwithAddress(temp_block, lucky_ticket.ticketAddress.ToString(), i))
				{
					LogPrint(BCLog::TICKETPOOL,"CheckSigwithAddress OK ，index is ：%d \n",i);
				}
				else
				{
					LogPrint(BCLog::TICKETPOOL,"CheckSigwithAddress failed ，index is ： %d\n",i);
					LogPrint(BCLog::TICKETPOOL,"failed block ：%s\n",temp_block.GetHash().ToString());
					return false;
				}
				i++;
			}
		...
		if (ret) {
			...
			LogPrint(BCLog::NET,"AcceptBlock :%S \n",ret?"true":"false");
		}
		CheckBlockIndex(chainparams.GetConsensus());
		if (!ret) {
		...
		const CBlock& constblock = *pblock;
		//accept_block = const_cast<CBlock&>(constblock);
		//acceptHeight = pindex->nHeight;
	}
	...
	LogPrint(BCLog::NET,"updateLoadHeight :%d \n",pindex->nHeight);

	return true;
}                                                                                       //是强制更新头信息
bool ProcessNewMixedBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{	//GetMainSignals().NewPoWValidBlock(nullptr, pblock);
	GetMainSignals().NewMixedPoWValidBlock(nullptr, pblock,fForceProcessing);
    return true;
}
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    bool temp_ret=pblocktree->ReadFlag("txindex", fTxIndex);
	LogPrint(BCLog::FINDBEGIN,"temp_ret :%s   ,pblocktree->ReadFlag(\"txindex\", fTxIndex) ",temp_ret?"true":"false",fTxIndex?"true":"false");
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // tsc
        uint32_t sizeBlockDGP = tscDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
...
// block index state
//在任何连接启动后，可能不会使用，因为对等处理逻辑的大部分假设是一致的块索引状态。
void UnloadBlockIndex()
{
	LogPrint(BCLog::TICKETPOOL,"UnloadBlockIndex \n");
...
{	
    ...
    if (!fReindex) {
        ...
        bool ret = LoadBlockIndexDB(chainparams);
		LogPrint(BCLog::FINDBEGIN,"LoadBlockIndexDB:%s\n" , ret?"true":"false");
        if (!ret) return false;
        needs_init = mapBlockIndex.empty();

		LogPrint(BCLog::FINDBEGIN,"needs_init : %s\n",needs_init?"true":"false");
    }
    ...
    if (needs_init) {
		...
		LogPrint(BCLog::FINDBEGIN,"gArgs.GetBoolArg fTxIndex : %s\n",fTxIndex?"true":"false");

        bool temp_ret2=pblocktree->WriteFlag("txindex", fTxIndex);
		LogPrint(BCLog::FINDBEGIN,"WriteFlag : %s write fTxIndex) : %s\n",temp_ret2?"true":"false",fTxIndex?"true":"false");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    int nLoaded = 0;
    try {
        // 这将接管fileIn并在CBufferedFile析构函数中对其调用fclose()
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Tsc PoS we need to sync this after every block to ensure txdb is populated for
...
bool LoadTicketpool(void)
{
	while (!fTipOk)
	{
		std::cout << "!fTipOk" << std::endl;
        usleep(10000);
	}
	//std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" <<std:: endl;
	int64_t start = GetTimeMicros();
	...
	{
		times = chainActive.Tip()->nHeight;
		LogPrint(BCLog::TICKETPOOL,"times %d    temp_height %d\n",times,temp_height);
	...
	else
	{
		sleep(3);
		sleep(20);
		times = chainActive.Height();
		while (times != (mapBlockIndex.size() - 1))
		{
			times = chainActive.Height();
			sleep(1);
		}
	...
	for( int i = 1 ; i <= times ; i++)
	{
		...
		LogPrint(BCLog::RPC,"i =%d\n",i);

		//GETBLOCK(times);//获得第n个块
		if (times > chainActive.Height())
		{
			LogPrint(BCLog::TICKETPOOL,"Block height out of range\n");
			//throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");
		}

		//CBlockIndex* pblockindex = chainActive[times];

		uint256 blockhash = chainActive[i]->GetBlockHash();
		LogPrint(BCLog::TICKETPOOL,"blockhash  =%s\n",blockhash.ToString());

		CBlockIndex * pblockindex = mapBlockIndex[blockhash];
        LogPrint(BCLog::TICKETPOOL,"[LoadTicketpool] i = %d,ticketpool.size1 = %d\n",i,ticketpool.ticket_pool.size());
        updateTicketpool(pblockindex, i);
        LogPrint(BCLog::TICKETPOOL,"[LoadTicketpool] i = %d,ticketpool.size2 = %d\n",i,ticketpool.ticket_pool.size());

	}
	int64_t last = GetTimeMicros();
	LogPrint(BCLog::RPC,"update ok;\n");
	floadok = true;
	LogPrint(BCLog::TICKETPOOL,"LoadTicketpool cost = %jd s",(last- start)*0.000001);
	return true;
}

void updateTicketpool(CBlockIndex * pblockindex, int block_height)
{
    int ticket_height = block_height - TICKETPOOL_HEIGHT_LATENCY;
    if (ticket_height <= ticketpool.getLoadHeight())
    {
        //todo:分叉情况下块回退需要把票池中删除的票补回来
        if (block_height <= ticketpool.getLoadHeight())
        {
            ticketpool.ticketDelByHeight(block_height);
        }
        return ;
    }

	LogPrint(BCLog::TICKETPOOL,"============ updateTicketpool start =================== %d \n",block_height);
    CBlock block;
    CBlockIndex * ticketBlockIndex = chainActive[ticket_height];
    if (!ReadBlockFromDisk(block, ticketBlockIndex, Params().GetConsensus()))
    {
        LogPrint(BCLog::TICKETPOOL,"Block not found on disk \n");
        return;
    }

	// lucky ticket OK
	ticketPool temppool = ticketpool;
	if (block_height > TICKET_VALVE )
	{
		//delete previous lucky ticket 

		//get nowaday block sig ticket 
		LogPrint(BCLog::TICKETPOOL,"delete previous lucky ticket\n");

		std::vector<unsigned int> luckyindex = luckyticket.getLuckyIndex(block_height - 1,block.seed);
		std::vector<Ticket> vlucky_ticket;
		//std::cout<<" updateTicketpool block_height  "<<block_height<<std::endl;
		for (auto index : luckyindex)
		{
			Ticket lucky_ticket = temppool.ticketAt(index);
			vlucky_ticket.push_back(lucky_ticket);
			temppool.ticketDel(lucky_ticket); 
		}
		LogPrint(BCLog::TICKETPOOL," ticketpool.ticket_pool.size =%d\n ",ticketpool.ticket_pool.size());
		LogPrint(BCLog::TICKETPOOL," vlucky_ticket.size =%d\n ",vlucky_ticket.size());
		for (auto ticket : vlucky_ticket)
		{
			ticketpool.ticketDel(ticket);
		}
        LogPrint(BCLog::TICKETPOOL,"deleted ticketpool.ticket_pool.size =%d\n ",ticketpool.ticket_pool.size());
		LogPrint(BCLog::TICKETPOOL,"delete previous lucky ticket OK\n");
	}
	LogPrint(BCLog::TICKETPOOL,"--------------------------------------------------\n");
    // delete the most old ticket from ticketpool
	if (ticket_height >= TICKET_POOL_MAX_SIZE)
	{
		
		LogPrint(BCLog::TICKETPOOL,"delete the most old ticket from ticketpool\n");
        ticketpool.ticketDelByHeight(block_height - TICKET_POOL_MAX_SIZE);
        /*
		std::vector<Ticket> vHticket=ticketpool.getTicketByHeight(block_height-TICKET_POOL_MAX_SIZE );
		LogPrint(BCLog::TICKETPOOL,"block_height  %d\n",block_height);
		LogPrint(BCLog::TICKETPOOL,"block_height-TICKET_POOL_MAX_SIZE %d\n ",block_height - TICKET_POOL_MAX_SIZE);
		LogPrint(BCLog::TICKETPOOL,"vHticket.size %d\n",vHticket.size());
        LogPrint(BCLog::TICKETPOOL,"block.vtx = %d\n",block.vtx.size());
		for (auto tx : block.vtx)
		{
			if (tx->IsCoinBase())
			{
				LogPrint(BCLog::TICKETPOOL,"delete vout size:%d\n" , tx->vout.size());
				if ((tx->vout.size() - 7) == vHticket.size())
				{
					ticketpool.ticketDelByHeight(block_height - TICKET_POOL_MAX_SIZE);
					LogPrint(BCLog::TICKETPOOL,"delete OK\n");
	...
	LogPrint(BCLog::TICKETPOOL,"===============================\n");
	std::vector<CTransactionRef> VticketTx;
	
	LogPrint(BCLog::TICKETPOOL,"buyticket ticketpool.ticket_pool.size() 1 = %d\n",ticketpool.ticket_pool.size());
	for (auto tx : block.vtx)
	{

		if ((tx->vout[0]).scriptPubKey.HasOpSStx())
		{
			VticketTx.push_back(tx);
			Ticket temp_ticket(*tx, ticket_height);
        
			ticketpool.ticketPush(temp_ticket);
		}
	}
    
	ticketpool.updateLoadHeight(ticket_height);
	LogPrint(BCLog::TICKETPOOL,"buyticket ticketpool.ticket_pool.size() 2 = %d\n",ticketpool.ticket_pool.size());
	LogPrint(BCLog::TICKETPOOL,"============ updateTicketpool end ===================\n");
