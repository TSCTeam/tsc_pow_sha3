...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
	...
	if (tx.vin.empty())
	{
		//std::cout << " vin empty" << std::endl;
        return state.DoS(10, false, REJECT_INVALID, "bad-txns-vin-empty");
	}
	if (tx.vout.empty())
	{
		//std::cout << " vout empty" << std::endl;
    ...
    for (const auto& txout : tx.vout)
    {
		//std::cout << txout.IsEmpty() << std::endl;
		//std::cout << !tx.IsCoinBase() << std::endl;
		//std::cout << !tx.IsCoinStake() << std::endl;
        if (txout.IsEmpty() && !tx.IsCoinBase() && !tx.IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-vout-empty");
		//std::cout << " 8.1.4.1" << std::endl;

        if (txout.nValue < 0)
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-vout-negative");
		//std::cout << " 8.1.4.2" << std::endl;

		if (txout.nValue > MAX_MONEY)
		{
		//	std::cout << "txout.nValue "<< txout.nValue<< "  | MAX_MONEY " << MAX_MONEY << std::endl;
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-vout-toolarge");
		}
		//std::cout << " 8.1.4.3" << std::endl;
//
        nValueOut += txout.nValue;
        if (!MoneyRange(nValueOut))
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-txouttotal-toolarge");
		//std::cout << " 8.1.4.5" << std::endl;
